...
- **Timezone trap on "today" detection** — the trip is in Asia/Tokyo. Any "today" check that uses `new Date().toISOString().slice(0, 10)` is **UTC**, which lags Tokyo by 9 hours — a Tokyo-morning page-load returns yesterday's date and surfaces the wrong day. Verify all "today" detection code uses an explicit `{ timeZone: 'Asia/Tokyo' }` formatter (e.g. `new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Tokyo' }).format(new Date())`). Also verify the detection runs **client-side**, not in Astro frontmatter — Astro frontmatter runs at build time on GitHub Actions in UTC and the rendered HTML is then cached. A static-site build is hours-to-days stale by the time the user opens it; only client-side JS sees the real current time.
