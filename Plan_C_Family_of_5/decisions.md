# Plan C — Decisions Log

Running list of locked-in decisions for the final plan. Supersedes Plan_A and Plan_B.

**Group:** 5 people — 2 adults (Ajay, Candice) + 2 kids (Zara, Kai) + grandma (Seck Joo Lee)
**Dates:** May 29 – June 8, 2026 (10 nights)
**Flights:** ZIPAIR booked — see [../bookings/flights.md](../bookings/flights.md)

---

## ✅ Decided

| # | Decision | Notes |
|---|----------|-------|
| 1 | **Group is 5** | 4 SJC round-trip + grandma SJC→NRT, grandma departs NRT→SIN early morning June 8 |
| 2 | **All arrive together** | Row 51 on ZG029, NRT 14:50 on May 29 |
| 3 | **Last night in Narita City** | Not airport-terminal hotel. Temple town vibe, unagi dinner. Grandma solo 5:30am taxi to NRT June 8. |
| 4 | **No DisneySea** | Family has never done Disney — too much time/$ to commit to an unknown format with grandma in tow. Japan has enough unique magic without it. |
| 5 | **Food guide carried over** | [food.md](food.md) — family's general list of foods they're good with. Applies trip-wide. |
| 6 | **Pacing: active mornings / slow afternoons / interesting evenings** | Grandma is active; no need for slow mornings. |
| 7 | **Skip temple-contemplative stuff** | Meiji Shrine, Shinjuku Gyoen, Kiyomizudera, Philosopher's Path all cut. Keep only visually/experientially driven "traditional" sites: Fushimi Inari, Todai-ji Great Buddha, Naritasan. |
| 8 | **City split: Tokyo 4 / Hakone 2 / Kyoto 3 / Narita 1 = 10 nights** | Kyoto is base for Nara + Osaka day trips; no Osaka overnight |
| 9 | **Day trips from Kyoto** | Nara (deer + Todai-ji) as half day. Osaka Dotonbori on the Kyoto→Narita travel day. |
| 10 | **No JR Pass** | Individual Shinkansen tickets cheaper for this route count. Use Suica for everything local. |
| 11 | **teamLab Planets Tokyo** (Toyosu) | ~~Borderless~~ reversed. Booked Jun 1, 14:00–14:30 entry, 3 adults + 2 kids. Barefoot + ankle-to-knee water installations — kids love it; grandma can do the dry rooms or sit out. See [bookings/teamlab.md](../bookings/teamlab.md). |
| 12 | **No PokéPark KANTO** | Cut. Day 3 needs a replacement. |
| 13 | **Day 10 is a chill day, no Osaka push** | Candice haircut, souvenir/last-minute shopping in Kyoto, coffee, relaxed Shinkansen+NEX travel to Narita. |
| 14 | **Grandma SIN flight: 08:20 NRT, June 8** | Booked on Scoot TR 885 (Terminal 1). 5:30am taxi plan fits. |
| 15 | **Hakone lodging: Rakuten STAY TERRACE Hakone Kowakudani** | Booking `2442810403`. 483-2 Kowakudani. Whole-unit apartment, Stayable Showroom, non-smoking. Solves no-nude filter via Yunessun walkable. Online check-in URL sent day before; reply with email address to confirm. See [bookings/hakone_lodging.md](../bookings/hakone_lodging.md). |
| 16 | **Tokyo lodging: Airbnb — (#3) Luxury Condo in Shinjuku** | 4-chōme-27-3 Yotsuya, Shinjuku-ku. Confirmation `HMYZXN9NPB`. Superhost (388 reviews @ 4.81). 1BR/4 beds — verify sleep arrangement on arrival. Yotsuya 4-chome = east of Shinjuku Gyoen, walkable to Shinjuku Station (Romancecar). See [bookings/tokyo_lodging.md](../bookings/tokyo_lodging.md). |
| 17 | **Narita City lodging: Richmond Hotel Narita** | Trip ID `1016631380`. 2 rooms × 1 night (Jun 7–8): Twin+Sofa (conf `2445218331`, $125) + Deluxe Double (conf `2445218289`, $101). Walking distance to Omotesando/Kawatoyo unagi. Book grandma's 5:30 AM taxi at reception on arrival. See [bookings/narita_lodging.md](../bookings/narita_lodging.md). |
| 18 | **Kyoto lodging: STITCH HOTEL Kyoto** | Confirmation `6410484352`, PIN `3605`. Apartment Suite 2nd floor, 624 sq ft, 2 full beds + 1 bunk = sleeps 5. Shimogyo-ku/Kawaramachi area — Hankyu Kawaramachi 2-min, Keihan Gion-Shijo 2-min (direct to Fushimi Inari). 3 nights ¥289,988 incl. tax. Free cancellation until May 27, 2026. Front desk handles takkyubin onward to Narita. See [bookings/kyoto_lodging.md](../bookings/kyoto_lodging.md). |
| 19 | **Cut Nintendo Museum** | Family not interested. Day 8 (Fri Jun 5) becomes a fully flex Kyoto day — likely K1 Fushimi Inari Sunrise + K2 or K4 + KE1 Gion or KE3 Ramen Koji. |

---

## 📅 Itinerary

See [itinerary.md](itinerary.md) for the full day-by-day plan with activities, transport, and bookings.

---

## 🤔 Open Questions

- ~~Hotel selections: all 4 cities done~~ ✅
- ~~Grandma's NRT → SIN flight: airline, flight #, confirmation, terminal (time = 08:20 June 8)~~ ✅ Booked (Scoot TR 885, Terminal 1)
- **Day 3 replacement activity** (PokéPark is cut). Options below.
- Whether 4 Tokyo nights is still right now that PokéPark is out, or drop to 3.
