# 🏨 Narita City Lodging — ✅ BOOKED

**Trip ID:** `1016631380`
**Platform:** Expedia (or similar OTA — confirm from receipt)
**Booked:** 2026-04-23

---

## Property

- **Name:** Richmond Hotel Narita
- **Location:** Narita City (central — walking distance to Omotesando Street + Naritasan)
- **Type:** Business hotel
- **Nights:** 1 (Sun Jun 7 → Mon Jun 8, 2026)
- **Check-in:** 2:00 PM
- **Check-out:** 11:00 AM
- **Rooms:** 2

---

## Room 1 — Twin Room (2 Single Bed + 1 Sofa Bed)

- **Confirmation #:** `2445218331`
- **Primary guest:** Ajay M Nainani
- **Capacity booked:** 5 guests *(likely a booking-site artifact — actual sleep capacity is 3)*
- **Sleep config:** 2 single beds + 1 sofa bed = 3 sleeping spots
- **Smoking:** Non-smoking
- **Price:** $125.14 (incl. taxes/fees)
- **Free cancellation until:** Sun Jun 7, 2026, 5:59 PM (property local time)

## Room 2 — Deluxe Double Room

- **Confirmation #:** `2445218289`
- **Primary guest:** Candice Kao
- **Capacity booked:** 2 guests
- **Sleep config:** 1 double bed = 2 sleeping spots
- **Smoking:** Non-smoking
- **Price:** $100.73 (incl. taxes/fees)
- **Free cancellation until:** Sun Jun 7, 2026, 5:59 PM (property local time)

**Total:** ~$225.87 for 2 rooms × 1 night

---

## Sleep Arrangement Plan

Total sleep capacity across 2 rooms = 5 (matches group size ✓)

**Proposed arrangement** (adjust as preferred):
- **Room 1 (Twin + sofa, 3 spots):** Grandma + 2 kids (Zara, Kai)
- **Room 2 (Deluxe Double, 2 spots):** Ajay + Candice

*Alternative if grandma wants privacy from the kids:*
- Room 1 (Twin + sofa): Ajay + Zara + Kai
- Room 2 (Double): Candice + Grandma

---

## ⚠️ At Check-In — Critical Tasks

1. **Book grandma's 5:30 AM taxi** to NRT at reception. Richmond Hotel does this routinely; tell them:
   - Pickup 5:30 AM from hotel entrance
   - Destination: Narita International Airport (NRT), **Terminal 1**
   - Solo passenger (grandma, Seck Joo Lee), 1 piece of luggage
   - Flight: **Scoot TR 885**, 08:20 NRT → SIN (Terminal 1), conf `R643SL`
2. **Mention early departure** — grandma leaving before standard check-out. Just leaves the key in the room or at reception.
3. **Confirm adjacent rooms** — if not already assigned, request rooms on the same floor.

---

## Location Notes

- **Walking distance to Omotesando Street** — the 800m pilgrimage road to Naritasan with wooden shops, festival vibe
- **Kawatoyo Honten** (unagi dinner, Day 10) — ~10-min walk from hotel. Arrive by 18:30 to beat the queue.
- **JR Narita Station / Keisei Narita Station** — both ~10 min walk
- **NRT airport** — 10 min by taxi (~¥2,500) or 10 min by Keisei line (¥260)

---

## Departure Morning (Jun 8)

### Grandma — 5:30 AM taxi
- Hotel → NRT **Terminal 1**: 10 min, ~¥2,500
- At terminal by ~5:40 AM for 08:20 Scoot TR 885 → 2h40 pre-flight ✓
- Scoot check-in opens 05:20 (3h prior), closes 07:20 (60 min prior)

### Family — Relaxed morning
- **Breakfast** at hotel (verify if breakfast plan was included; otherwise in lobby or nearby)
- **Optional Naritasan Park walk** ~09:00 (30 min)
- **Check out by 11:00 AM**
- **Lunch** — konbini bento or quick eat on Omotesando
- **12:30–13:00:** taxi or Keisei line to NRT
- **13:30:** at NRT for 16:25 ZG030 departure

---

## Booking Management

- **Trip ID:** 1016631380
- **Free cancellation:** until Sun Jun 7, 5:59 PM local time (extremely generous — no pressure)
- Change or cancel via the OTA booking site

---

## Flag for Follow-Up

- **Guest count on Room 1 shows "5 guests"** but sleep capacity is 3. Probably a booking artifact — verify with hotel 2–3 days before arrival that the rooms are correctly assigned and not flagged for overcrowding.
- ~~Grandma's NRT → SIN flight details~~ ✅ Captured: Scoot TR 885, NRT T1 → SIN T1, 08:20→14:35, conf `R643SL`. See [bookings/flights.md](flights.md#departure--scoot-r643sl-).
